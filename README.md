...
**NOTE: this is just a proof of concept for hashing out ideas** (Once we know better what functionality and performance we need, we plan to add that to 'git fit' and abandon this project; see 'Alternatives to git-dat' below.)
...
Note that if you are using a repository that already contains a git-dat.json, you will be prompted to create the data directory (e.g. ``Could not load config file git-dat.json: Invalid data path (does 'data' exist?)``) before you can run ``git dat init``.
